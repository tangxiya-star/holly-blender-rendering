...
def _build_materials():
    ...
    shoulder = _map(nt, separate.outputs['Z'], .044, .062, 0, .85,
                    'Powder deposit follows the upper shoulder', (-230, 640))
    powder_distribution = _noise_node(nt, tx, 'Asymmetric powder deposits · 7 mm',
                                     145, 2, .64, (-800, 760))
    patch = _map(nt, powder_distribution.outputs['Fac'], .34, .68, .02, 1.0,
    ...
    dust.inputs[2].default_value = (.11, .235, .020, 1)
    nt.links.new(dust.outputs['Color'], bs.inputs['Base Color'])
    rough = _map(nt, pore.outputs['Fac'], .15, .84, .48, .72, 'Crust absorbs unevenly')
    ...
    mats['powder'] = []
    for i, c in enumerate([(.095, .19, .014), (.175, .285, .023), (.06, .125, .005), (.29, .355, .038)]):
    ...
    for i, (c, r) in enumerate([
        ...
        nt.links.new(br.outputs['Normal'], bs.inputs['Normal'])
        mats['roe'].append(m)
    mats['roe_film'] = _mat('Food | localized roe moisture meniscus',
                           (.006, .004, .002), .105, .015, .00008, .04, 1.34)[0]
...
_PROFILE = [(0, .68), (.04, .81), (.12, .912), (.23, .970),
...
def _pastry_point(t, a, center, radius, z0, height, seed, relief=True):
    ...
    # Different broad, gently off-center baked forms for the two pastries.
    # This low-frequency imbalance is independent of the fine skin detail.
    middle = math.sin(math.pi*t)**.75
    r *= 1 + middle*(.026*math.cos(a-phase*.83)+.020*math.cos(2*a+phase*.47))
    z += .00075*math.sin(a+phase*.61)*math.sin(math.pi*t)**1.35
...
def _pastry(name, center, radius, z0, body_height, seed, collection, mats):
    ...
    fv, ff, fi = [], [], []
    blister_patches=[(rng.uniform(.14,.88),rng.uniform(0,TAU)) for _ in range(44)]
    for k in range(int(3900*FOOD_DETAIL)):
        if rng.random()<.47:
            pt,pa=rng.choice(blister_patches)
            t=max(.06,min(.98,rng.gauss(pt,.11)))
            a=rng.gauss(pa,.38)%TAU
        else:
            t = rng.uniform(.065, .975)
            a = rng.uniform(0, TAU)
        p = Vector(_pastry_point(t, a, center, radius, z0, body_height, seed))
        skin_patch=max(.05,min(.95,.5+.75*_noise((p.x+seed*.019,p.y,p.z),105)))
        if rng.random() > .40+.60*skin_patch:
            continue
        ...
        sx = rng.uniform(.00050, .00132)
        sy = sx*rng.uniform(.58, 1.10)
        h = rng.uniform(.00010, .00030)*(.48+skin_patch)
    ...
    gv, gf, gi = [], [], []
    for k in range(int(34000*FOOD_DETAIL)):
        t = rng.uniform(.45, .985)
        a = rng.uniform(0, TAU)
        p = Vector(_pastry_point(t, a, center, radius, z0, body_height, seed))
        patch = _noise((p.x+seed*.006, p.y, p.z), 330)
        if rng.random() > (.36+.50*(t-.45)/.535 + .40*patch):
            continue
        grain = rng.uniform(.000080, .000240)
        tangent = Vector((-math.sin(a), math.cos(a), 0))
        up = Vector(_pastry_point(min(.999, t+.002), a, center, radius, z0, body_height, seed, False))-Vector(_pastry_point(max(.001, t-.002), a, center, radius, z0, body_height, seed, False))
        up.normalize()
        normal = tangent.cross(up).normalized()
    ...
    assert min(dust_z) > z0+body_height*.40, 'Shoulder dust is below its intended surface region'
    assert max(dust_z) > z0+body_height*.92, 'Shoulder dust is missing from the crown'
...
def _append_roe(verts, faces, indices, center, r, rng, matidx, detail=1.0):
    ...
    ax, ay, az = rng.uniform(.905,1.105), rng.uniform(.94,1.055), rng.uniform(.815,1.035)
    rotation=phase*2.17
    ...
    for j in range(1,V):
        ...
        for i in range(U):
            a=TAU*i/U
            irregular=1+.023*math.sin(3*a+phase)*st**2+.012*math.cos(theta*4+phase)*st
            da=abs((a-dimple_a+math.pi)%TAU-math.pi)
            dimple=.036*math.exp(-((theta-dimple_t)/.19)**2-(da/.3)**2)
            rr=r*(irregular-dimple)
            lx,ly=rr*st*math.cos(a)*ax,rr*st*math.sin(a)*ay
            verts.append((center[0]+lx*math.cos(rotation)-ly*math.sin(rotation),
                          center[1]+lx*math.sin(rotation)+ly*math.cos(rotation), center[2]+rr*ct*az))
    for j in range(V-2):
        for i in range(U):
...
def _roe_cluster(name, center, radius, base, seed, collection, mats):
    ...
    # Sparse real menisci at existing bead contacts. These small saddle-shaped
    # bridges interrupt uniform bead highlights without laying a glaze over roe.
    candidates=[]
    for i,e1 in enumerate(eggs):
        for j in range(i+1,len(eggs)):
            e2=eggs[j]
            if min(e1[2],e2[2])<base+.0025:
                continue
            distance=(Vector(e2[:3])-Vector(e1[:3])).length
            gap=distance-e1[3]-e2[3]
            if -.00016<gap<.00016:
                candidates.append((e1,e2,gap))
    rng.shuffle(candidates)
    mv,mf=[],[]
    for e1,e2,gap in candidates[:22]:
        p1,p2=Vector(e1[:3]),Vector(e2[:3])
        axis=(p2-p1).normalized()
        perpendicular=axis.cross(Vector((0,0,1)))
        if perpendicular.length<.01:
            perpendicular=axis.cross(Vector((0,1,0)))
        perpendicular.normalize()
        other=axis.cross(perpendicular).normalized()
        contact=p1+(p2-p1)*(e1[3]/(e1[3]+e2[3]))
        half_length=max(.00018,gap*.5+.00018)
        neck=rng.uniform(.00011,.00020)
        off=len(mv)
        for j in range(7):
            t=-1+2*j/6
            rr=neck+.00018*t*t
            for i in range(12):
                a=TAU*i/12
                p=contact+axis*(t*half_length)+perpendicular*(math.cos(a)*rr)+other*(math.sin(a)*rr)
                mv.append(tuple(p))
        for j in range(6):
            for i in range(12):
                mf.append((off+j*12+i,off+j*12+(i+1)%12,off+(j+1)*12+(i+1)%12,off+(j+1)*12+i))
    if mf:
        moisture=_mesh(name+' | sparse moisture at roe contacts',mv,mf,collection,[mats['roe_film']])
        moisture['meniscus_count']=min(22,len(candidates))
    return ob,eggs
...
def _flower(name, position, radius, seed, collection, mats, tilt=(.12,-.33,.0)):
    ...
    for petal in range(4):
        angle=TAU*petal/4+rng.uniform(-.25,.25)+seed*.17
        ...
        fold=rng.uniform(.20,.37) if petal==1 else (rng.uniform(-.22,-.12) if petal==3 else rng.uniform(-.07,.14))
