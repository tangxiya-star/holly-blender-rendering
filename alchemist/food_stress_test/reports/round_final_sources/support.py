...
def _moss(col, mats):
    ...
    # A few higher, narrow cushion crests interrupt the general low mound band.
    mounds.extend([(-.050,-.041,.0088,.0046),(-.036,-.063,.0083,.0052),
                   (.034,-.061,.0087,.0048),(-.063,.004,.0081,.005)])
    def surface(x,y):
    ...
    for cx,cy,spread,height,number in clusters:
        for kk in range(number):
            ...
            regional_scale=.98+.60*noise(Vector((x*82,y*82,19)))
            ...
            for j in range(6):
                ...
                for sign in [-1,1]:
                    ...
                    length=rng.uniform(.0011,.0023)*(1-.26*t)*regional_scale
    ...
    for fi in range(420):
        ...
        length=rng.uniform(.006,.014);rise=rng.uniform(.001,.0045);curl=rng.uniform(-.005,.005)
        wave=rng.uniform(.66,1.6);regional_scale=.98+.60*noise(Vector((x*82,y*82,19)))
        def path(t):
            return root+forward*(length*t)+side*(curl*math.sin(t*math.pi*wave))+Vector((0,0,rise*math.sin(t*math.pi*rng_phase)))
        rng_phase=rng.uniform(.62,1.3)
        points=[path(j/9) for j in range(10)]
        idx=rng.choice([0,2,4,5])
        tube(points,rng.uniform(.00013,.00022),idx)
        leaf_count=rng.randint(9,15)
        for j in range(leaf_count):
            t=.08+.84*j/(leaf_count-1)+rng.uniform(-.015,.015)
            for sign in [-1,1]:
                p=path(t+rng.uniform(-.013,.013))
                direct=(side*sign+forward*rng.uniform(-.3,.65)+Vector((0,0,rng.uniform(-.2,.45))))
                ll=rng.uniform(.0009,.0022)*(1-.3*t)*regional_scale
                leaf(p,direct,ll,ll*rng.uniform(.40,.63),rng.uniform(.0,.60),idx)
        for t,sign in [(rng.uniform(.22,.46),-1),(rng.uniform(.53,.76),1)]:
...
def _slate(col,mats):
    ...
    obj['reference_rear_edge']='y = .165 - .90*x; front y=-.7; x=+/- .7 metres'
