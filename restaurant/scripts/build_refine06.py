...
parser=argparse.ArgumentParser();parser.add_argument('--render',default='entry');parser.add_argument('--load');parser.add_argument('--quality',action='store_true');parser.add_argument('--pass-b',action='store_true');parser.add_argument('--pass-c',action='store_true')
a=parser.parse_args(sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [])
if a.pass_c:a.pass_b=True
start=time.time()
...
if a.load:
    ...
    if a.pass_c:
        from refine06_microtexture import apply_microtexture
        report['microtexture']=apply_microtexture(s)
    ...
    outname='alchemist_06c_refine.blend' if a.pass_c else 'alchemist_06b_refine.blend' if a.pass_b else 'alchemist_06a_refine.blend'
    ...
    (R/('reports/refine06c_build.json' if a.pass_c else 'reports/refine06b_build.json' if a.pass_b else 'reports/refine06_build.json')).write_text(json.dumps(report,indent=2))
    snap=R/('reports/refine06c_sources' if a.pass_c else 'reports/refine06b_sources' if a.pass_b else 'reports/refine06_sources');snap.mkdir(exist_ok=True)
    for name in ['build_refine06.py','refine06_surfaces.py','refine06_door.py','refine06_furniture.py','refine06_entry_finish.py','refine06_microtexture.py']:shutil.copyfile(R/'scripts'/name,snap/name)
...
for key in a.render.split(','):
    ...
    s.render.image_settings.file_format='PNG';s.render.image_settings.color_depth='16'
    tag=('final' if a.quality else 'review')+('_c' if a.pass_c else '_b' if a.pass_b else '')
