...
class _Geometry:
    def __init__(self):
        ...
        self.slots = []
        self.flat = []
        self.bark_coordinate = []

    def vert(self, co, wear=.1, cavity=0, bark_coordinate=None):
        self.vertices.append(co)
        self.wear.append(min(1, max(0, wear)))
        self.cavity.append(min(1, max(0, cavity)))
        self.bark_coordinate.append(bark_coordinate if bark_coordinate is not None else (co[2],co[0],0))
    ...
    def sweep(self, points, width, seed, detail=40, center=-.049, flat=.50,
        ...
        points = _sample(points, .0035 if width>.05 else .0045)
        if bark:
            detail=max(detail,192 if width>.08 else 144 if width>.045 else 96 if width>.018 else 64)
        offset = len(self.vertices)
        rand = random.Random(seed)
        phase = rand.random()*math.tau
        lobes = max(9,round(math.tau*width/rand.uniform(.014,.019)))
        arc=[0.0]
        for a,b in zip(points,points[1:]):arc.append(arc[-1]+math.hypot(b[0]-a[0],b[1]-a[1]))
        for i, (x, z, scale) in enumerate(points):
            ...
            for j in range(detail):
                a = j*math.tau/detail
                along=arc[i];across=a*width
                # Continuous branched grain in physical unwrapped coordinates.
                # Coarse low-frequency bending makes the creases follow a
                # twisting trunk; interacting narrow ridges split and terminate.
                warp=noise(Vector((across*22,along*7,seed*.13)))*3.1
                warp+=.45*math.sin(along*21+phase)
                grain_phase=a*lobes+phase+warp
                ridge=math.sin(grain_phase)
                split=math.sin(grain_phase*.61+along*9+noise(Vector((across*11,along*4,seed)))*2)
                furrow=(.5+.5*ridge)**3.8
                groove=max(0,-ridge)**7
                grain=noise(Vector((across*100,along*46,seed*.37)))
                interruption=.55+.45*noise(Vector((across*36,along*17,seed+53)))
                broken=(.0080*furrow*(.65+.35*split)*interruption-.0045*groove)
                broken+=.0016*grain+.0010*noise(Vector((across*220,along*91,seed)))
                broken*=min(1,width/.038)*end
                corr=1+.027*math.sin(a*5+warp) if bark else 1+.02*ridge
                nv=noise_vector(Vector((x*30+j*.015,z*30,seed*.37)))
                ...
                if bark:
                    y-=broken*max(.1,abs(front))**.35
                    lateral += broken*.45*math.cos(a)+.0013*nv.x
                self.vert((x+nx*lateral, y, z+nz*lateral),
                          .08+.48*max(0,front)*furrow,
                          .08+.78*groove*max(0,front),
                          bark_coordinate=(along,across,float(seed)))
        for i in range(len(points)-1):
            for j in range(detail):
                a=offset+i*detail+j
    ...
    def broken_bark(self, offset, stations, rings, points, width, seed):
        ...
        plate_count=max(8,int(area/.00040))
        ...
        def skin(t,a,coordinates=False):
            ...
            source=self.bark_coordinate if coordinates else self.vertices
            v=[source[i] for i in ids]
        ...
        for n in range(plate_count):
            ...
            physical_length=rand.uniform(.025,.082)*(1 if width>.06 else .72)
            ...
            physical_width=rand.uniform(.0013,.0045)
            da=min(.54,physical_width/max(.012,width))
            # Small directional meander follows the grain. Large random slants
            # in06b made isolated leaf-shaped chips, contrary to the reference.
            slant=rand.uniform(-.045,.045)
            height=rand.uniform(.0005,.0017)
            row_count=9;cols=3;begin=len(self.vertices)
            jagged=[rand.uniform(.80,1.12) for _ in range(row_count)]
            phase=rand.random()*math.tau
            for i in range(row_count):
                ...
                breadth=(.34+.66*math.sin(math.pi*t)**.32)*jagged[i]
                for j in range(cols):
                    cross=j/(cols-1)*2-1
                    angle=ac+slant*(t-.5)+cross*da*.5*breadth+.013*math.sin(t*7+phase)
                    st=t0+dt*t+cross*dt*.012*math.sin(t*7+phase)
                    x,y,z=skin(max(.001,min(.999,st)),angle)
                    lift=height*math.sin(math.pi*t)**.35*(.22+.78*(1-abs(cross))**.55)
                    # Torn transverse edge and one raised lip expose a deep
                    # narrow shadow gap beneath each fragment's local rim.
                    lip=height*.18*max(0,cross)*math.sin(math.pi*t)
                    wobble=.00013*math.sin(i*2.71+j*3.9+phase)
                    self.vert((x,y-.00020-lift-lip+wobble,z),
                              .12+.31*(1-abs(cross))*(.35+.65*t),
                              .1+.40*abs(cross),
                              bark_coordinate=skin(max(.001,min(.999,st)),angle,coordinates=True))
            for i in range(row_count-1):
                for j in range(cols-1):
                    a=begin+i*cols+j
                    self.face((a,a+1,a+cols+1,a+cols),0)
            ...
            for vi in boundary:
                ...
                bottom.append(self.vert((x,y+.00065,z),.03,.75,self.bark_coordinate[vi]))
...
def build_door(materials):
    ...
    def make(label,g,hinge,smooth=True):
        ...
        coords=mesh.attributes.new('bark_coordinate','FLOAT_VECTOR','POINT')
        coords.data.foreach_set('vector',[value for xyz in g.bark_coordinate for value in xyz])
    ...
    return {'objects':[o.name for o in created],'hinges':[h.name for h in hinges],
            ...
            'attributes':['bronze_wear','bronze_cavity','bark_coordinate'],
