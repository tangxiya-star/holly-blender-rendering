...
class _Geometry:
    ...
    def sweep(self, points, width, seed, detail=40, center=-.049, flat=.50,
        ...
        for i, (x, z, scale) in enumerate(points):
            ...
            t = i/(len(points)-1)
            end = min(1, .18+t*15, .15+(1-t)*18) if taper else 1
...
def build_door(materials):
    ...
    def make(label,g,hinge,smooth=True):
        ...
        for poly,slot in zip(mesh.polygons,g.slots):
            poly.material_index=slot;poly.use_smooth=smooth and len(poly.vertices)==4
    ...
    for side,hinge_x in (('Left',-1.6),('Right',1.6)):
        ...
        for idx,(width,raw) in enumerate(network):
            ...
            branch_count+=1
            rand=random.Random(seed)
            # Long, narrow raised lamellae supply geometric bark ridges and splits.
            for j in range(11 if width>.08 else 5):
                relief.bark_sliver(samples,rand.uniform(.0016,.0045),seed+j*13,width)
                slivers+=1
