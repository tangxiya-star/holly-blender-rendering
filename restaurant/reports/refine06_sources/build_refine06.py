...
parser=argparse.ArgumentParser();parser.add_argument('--render',default='entry');parser.add_argument('--load');parser.add_argument('--quality',action='store_true')
...
if a.load:
    ...
    s['door_dimensions']='4.15m leaf height,3.2m opening width retained for existing inferred scene. Width not a site measurement.'
    ...
    bpy.ops.file.pack_all();bpy.ops.wm.save_as_mainfile(filepath=str(R/'alchemist_06a_refine.blend'))
    ...
    (R/'reports/refine06_build.json').write_text(json.dumps(report,indent=2))
    snap=R/'reports/refine06_sources';snap.mkdir(exist_ok=True)
    for name in ['build_refine06.py','refine06_surfaces.py','refine06_door.py','refine06_furniture.py']:shutil.copyfile(R/'scripts'/name,snap/name)
...
for key in a.render.split(','):
    ...
    s.render.image_settings.file_format='PNG';s.render.image_settings.color_depth='16'
    tag='final' if a.quality else 'review'
