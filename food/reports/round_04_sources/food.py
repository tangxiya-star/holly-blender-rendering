...
def _build_materials():
    ...
    shoulder = _map(nt, separate.outputs['Z'], .041, .060, 0, 1.0,
                    'Powder deposit follows the upper shoulder', (-230, 640))
    patch = _map(nt, broad.outputs['Fac'], .22, .76, .30, 1.0,
...
def _pastry(name, center, radius, z0, body_height, seed, collection, mats):
    ...
    for k in range(int(3300*FOOD_DETAIL)):
        if rng.random()<.47:
