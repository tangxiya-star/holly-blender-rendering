...
def _build_materials():
    ...
    shoulder = _map(nt, separate.outputs['Z'], .051, .069, 0, .91,
...
def _pastry(name, center, radius, z0, height, seed, collection, mats):
    N, M = int(416*FOOD_DETAIL), int(164*FOOD_DETAIL)
    verts, faces, ids = [], [], []
    for k in range(M+1):
        t = k/M
        for i in range(N):
            verts.append(_pastry_point(t, TAU*i/N, center, radius, z0, height, seed))
    for k in range(M):
        t = (k+.5)/M
        for i in range(N):
            a = TAU*(i+.5)/N
    ...
    verts.extend([(center[0], center[1], z0), (center[0], center[1], z0+height-.0005)])
    for i in range(N):
    ...
    for k in range(int(1600*FOOD_DETAIL)):
        t = rng.uniform(.08, .97)
        a = rng.uniform(0, TAU)
        p = Vector(_pastry_point(t, a, center, radius, z0, height, seed))
        tangent = Vector((-math.sin(a), math.cos(a), 0))
        up = Vector(_pastry_point(min(.999, t+.002), a, center, radius, z0, height, seed, False))-Vector(_pastry_point(max(.001, t-.002), a, center, radius, z0, height, seed, False))
        up.normalize()
        ...
        for ring,height in [(.46,h*.83),(.78,h*.38),(1,-.00007)]:
            for i in range(seg):
                ang = TAU*i/seg
                rad = irregular[i]*ring
                fv.append(tuple(p+tangent*(math.cos(ang)*sx*rad)+up*(math.sin(ang)*sy*rad)+normal*height))
        for i in range(seg):
...
def _flower(name, position, radius, seed, collection, mats, tilt=(.12,-.33,.0)):
    ...
    for petal in range(4):
        angle=TAU*petal/4+rng.uniform(-.09,.09)+seed*.17
        along=(east*math.cos(angle)+north*math.sin(angle)).normalized()
        across=normal.cross(along).normalized()
        length=radius*rng.uniform(.89,1.08)
        width=radius*rng.uniform(.43,.54)
        off=len(verts)
        rows,cols=18,14
        for j in range(rows+1):
            ...
            for i in range(cols+1):
                ...
                cup=radius*(.095*s*s+.045*math.sin(t*math.pi)) + wav
...
def build_food(collection):
    ...
    specs=[
        dict(name='Hero front bite', center=(.003,-.026), radius=.0345, z0=.025, height=.047,
             cr=.0230, cb=.0741, ct=.0840, seed=43),
        dict(name='Rear bite', center=(-.032,.031), radius=.0295, z0=.025, height=.044,
             cr=.0205, cb=.0711, ct=.0805, seed=101),
    ]
    for spec in specs:
