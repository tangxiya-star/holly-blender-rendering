...
def _vein_image():
    ...
    def grow(start,finish,thickness,strength,seed,depth=3):
        ...
        if depth>0:
            for k in range(rr.randint(2,3)):
                ...
                side=rr.choice((-1,1))
                downward=rr.uniform(.035,.17) if depth>1 else rr.uniform(-.025,.065)
    ...
    for j in range(37):
        ...
        grow(start,finish,rng.uniform(.72,1.32),rng.uniform(.55,.95),1001+j,3)
    # A few longer fine capillaries from lower body quadrants, not a uniform halo.
    for j in range(9):
        u=rng.random()
        grow((u,.35),(u+rng.uniform(-.030,.030),.08),.68,.43,503+j,2)
...
def _materials():
    ...
    lower.inputs['From Min'].default_value=.006;lower.inputs['From Max'].default_value=.070
    lower.inputs['To Min'].default_value=.67;lower.inputs['To Max'].default_value=0
    ...
    amount=n.new('ShaderNodeMath');amount.operation='MULTIPLY';l.new(lower.outputs['Result'],amount.inputs[0]);l.new(mottles.outputs['Fac'],amount.inputs[1])
    rose=n.new('ShaderNodeMixRGB');rose.inputs[2].default_value=(.55,.16,.22,1)
    l.new(amount.outputs[0],rose.inputs[0]);l.new(mix.outputs[0],rose.inputs[1]);l.new(rose.outputs[0],b.inputs['Base Color'])
    ...
    iris,n,l,b=_material('Eye | gray-green gold radial stroma and crypts',(.30,.34,.28),.43,.025,.00023)
    ...
    bump=n.new('ShaderNodeBump');bump.inputs['Distance'].default_value=.00012;bump.inputs['Strength'].default_value=.19
    l.new(height.outputs['Color'],bump.inputs['Height']);l.new(bump.outputs['Normal'],b.inputs['Normal'])

    clear,n,l,b=_material('Eye | 65 micron transparent surface film',(.985,.99,.995),.21)
    b.inputs['Transmission Weight'].default_value=1;b.inputs['IOR'].default_value=1.34
    ...
    l.new(tx.outputs['Object'],ns.inputs['Vector'])
    rr=n.new('ShaderNodeMapRange');rr.inputs['From Min'].default_value=.2;rr.inputs['From Max'].default_value=.8;rr.inputs['To Min'].default_value=.18;rr.inputs['To Max'].default_value=.24
    l.new(ns.outputs['Fac'],rr.inputs['Value']);l.new(rr.outputs['Result'],b.inputs['Roughness'])
