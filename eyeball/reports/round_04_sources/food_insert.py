...
def _materials():
    ...
    roe=[]
    palette=[(.018,.010,.0044),(.027,.0155,.006),(.040,.024,.009),
             (.007,.0054,.0027),(.024,.017,.007),(.045,.025,.011)]
    for i,c in enumerate(palette):
        ...
        b.inputs['Transmission Weight'].default_value=.07
        ...
        b.inputs['Coat Weight'].default_value=.065 if i==5 else .012
        b.inputs['Coat Roughness'].default_value=.20 if i==5 else .34
...
def _roe(collection,materials,gel):
    ...
    for pindex,(x,y,radius) in enumerate(particles):
        ...
        ellipse=rng.uniform(.84,1.16)
        region=max(0,min(1,.5+.85*noise(Vector((x*190,y*190,17)))))
        zscale=rng.uniform(.36,.65)*(1-.17*region)
        immersion=max(.18,min(.64,.23+.36*region+rng.uniform(-.07,.07)))
        # Preserve the crowded visible footprint while flattening and burying
        # grain bodies to different depths in coherent patches of food/gel.
        contact_scale=.97/math.sqrt(1-immersion*immersion)
        base=_fluid_height(x,y)
        cz=base-radius*zscale*immersion
        weights=[5,5,3,6,4,1] if region<.62 else [4,3,2,11,5,.4]
    ...
    obj['visible_geometry']='Flattened custom lobed grains, regionally varying 18–64 percent radial immersion, crowded preserved cap footprint, and only a minority of clean wet membrane highlights.'
