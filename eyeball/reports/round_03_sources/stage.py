...
def material():
    ...
    low=nz(34,5);mid=nz(450,5);fine=nz(2600,3)
    color=ramp(low.outputs['Fac'],[.18,.48,.8],[(.105,.089,.073),(.22,.20,.17),(.35,.33,.285)])
    l.new(color.outputs[0],b.inputs['Base Color'])
    pits=ramp(mid.outputs['Fac'],[.32,.48,.53,.68],[(.03,)*3,(.08,)*3,(.66,)*3,(.76,)*3])
    bump=n.new('ShaderNodeBump');bump.inputs['Distance'].default_value=.00048;bump.inputs['Strength'].default_value=.7
