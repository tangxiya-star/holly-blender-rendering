...
def material():
    ...
    weathering=ramp(depth.outputs['Fac'],[0,.35,1],[(1.02,1.02,1.01),(.91,.91,.89),(.77,.79,.75)])
    mixed=n.new('ShaderNodeMixRGB');mixed.blend_type='MULTIPLY';mixed.inputs[0].default_value=1
    l.new(color.outputs[0],mixed.inputs[1]);l.new(weathering.outputs[0],mixed.inputs[2])
    # Irregular 0.5–3 mm pits in patchy clusters restore the broken mineral scale
    # between the broad physical scars and fine grain. Two thresholds vary with
    # local weathering, avoiding a uniform contour/network pattern.
    density=nz(63,3);chips=nz(1050,5);broken=nz(470,4)
    bias=n.new('ShaderNodeMath');bias.operation='MULTIPLY';bias.inputs[1].default_value=.38
    l.new(density.outputs['Fac'],bias.inputs[0])
    threshold=n.new('ShaderNodeMath');threshold.operation='ADD'
    l.new(chips.outputs['Fac'],threshold.inputs[0]);l.new(bias.outputs[0],threshold.inputs[1])
    chip_height=ramp(threshold.outputs[0],[.50,.575,.625,.70],[(.0,)*3,(.03,)*3,(.97,)*3,(1.,)*3])
    broken_height=ramp(broken.outputs['Fac'],[.27,.35,.43,.52],[(.06,)*3,(.12,)*3,(.88,)*3,(1.,)*3])
    height_mix=n.new('ShaderNodeMixRGB');height_mix.blend_type='MULTIPLY';height_mix.inputs[0].default_value=.5
    l.new(chip_height.outputs[0],height_mix.inputs[1]);l.new(broken_height.outputs[0],height_mix.inputs[2])
    mineral=ramp(height_mix.outputs[0],[.05,.32,.68,1],[(.18,.19,.17),(.44,.46,.42),(.86,.88,.84),(1.03,1.03,1)])
    color_mix=n.new('ShaderNodeMixRGB');color_mix.blend_type='MULTIPLY';color_mix.inputs[0].default_value=1
    l.new(mixed.outputs[0],color_mix.inputs[1]);l.new(mineral.outputs[0],color_mix.inputs[2]);l.new(color_mix.outputs[0],b.inputs['Base Color'])
    bump=n.new('ShaderNodeBump');bump.inputs['Distance'].default_value=.00065;bump.inputs['Strength'].default_value=.85
    l.new(height_mix.outputs[0],bump.inputs['Height'])
    micro=n.new('ShaderNodeBump');micro.inputs['Distance'].default_value=.00008;micro.inputs['Strength'].default_value=.4
    ...
    rough=ramp(height_mix.outputs[0],[.05,.5,1],[(.78,)*3,(.58,)*3,(.39,)*3]);l.new(rough.outputs[0],b.inputs['Roughness'])
    return m
...
def build_stage(collection):
    ...
    z=-.00010+contact*(micro_relief+lip*.5-recess*.5)
