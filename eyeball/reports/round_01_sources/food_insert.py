...
def _materials():
    ...
    m,ns,ls,b=_material('Eye food | dark viscous liquid and local menisci',(.0075,.0027,.0012),.23)
    b.inputs['IOR'].default_value=1.34;b.inputs['Transmission Weight'].default_value=.18
    b.inputs['Coat Weight'].default_value=.12;b.inputs['Coat Roughness'].default_value=.17
    tc=ns.new('ShaderNodeTexCoord');n=_noise(ns,ls,tc.outputs['Object'],950,3)
    rr=_ramp(ns,ls,n.outputs['Fac'],(.17,)*3,(.32,)*3)
    ls.new(rr.outputs['Color'],b.inputs['Roughness'])
    cr=_ramp(ns,ls,n.outputs['Fac'],(.0035,.0016,.001),(.013,.0055,.002))
    ls.new(cr.outputs['Color'],b.inputs['Base Color'])
    gel=m

    roe=[]
    palette=[(.027,.010,.0037),(.041,.016,.0058),(.058,.025,.009),
             (.019,.0082,.004),(.032,.017,.009),(.077,.035,.013)]
    for i,c in enumerate(palette):
        m,ns,ls,b=_material('Eye food | amber roe membrane %02d'%i,c,.29)
        ...
        b.inputs['Transmission Weight'].default_value=.14
        ...
        ls.new(cr.outputs['Color'],b.inputs['Base Color'])
        rr=_ramp(ns,ls,n.outputs['Fac'],(.20,)*3,(.36,)*3)
        ls.new(rr.outputs['Color'],b.inputs['Roughness'])
...
def _wall(collection,material):
    ...
    profile=[(.01455,.1020),(.0151,.1020),(.0153,.10225),
             (.01531,.10485),(.01529,.10535),(.01518,.10553),
             (.01496,.10557),(.01482,.1054),(.01475,.10515),(.01472,.10225)]
    ...
    obj['visible_wall_height_m']=.0035
    return obj
...
def _packing():
    ...
    # Sequential irregular packing, with slight overlaps confined below the liquid.
    for attempt in range(65000):
        if len(particles)>=176:break
        radius=rng.uniform(.00067,.00111)
        if rng.random()<.12:radius*=.79
        rad=(FOOD_RADIUS-radius*.84)*math.sqrt(rng.random());a=rng.uniform(0,TAU)
        x=rad*math.cos(a);y=rad*math.sin(a)
        if any((x-p[0])**2+(y-p[1])**2 < ((radius+p[2])*.92)**2 for p in particles):continue
...
def _roe(collection,materials,gel):
    ...
    for pindex,(x,y,radius) in enumerate(particles):
        ...
        immersion=rng.uniform(.18,.53)
        base=_fluid_height(x,y)
        cz=base-radius*zscale*immersion
        index=rng.choices(range(6),[4,5,4,3,2,1])[0]
        start=len(verts)
        ...
        for j,(extension,rise) in enumerate([(0,.000052),(.00012,.00002),(.00025,-.000012)]):
    ...
    obj['visible_geometry']='Unequal custom lobed closed grains; 18–53 percent radial immersion and nonuniform cap height.'
...
def build_food_insert(collection):
    ...
    return {'objects':len(objects),'roe_grains':grains['particle_count'],
            ...
            'insert_outer_radius_m':RIM_RADIUS,'insert_top_z_m':.10557,
