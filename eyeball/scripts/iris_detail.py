...
def _pigment_image(inner,outer,crypts):
    ...
    radial_pale=(1-v)**.55
    pale=np.clip(.36+.14*density+.35*radial_pale+.12*large-.16*v,.24,.88)
    blue=np.array((.045,.325,.415),dtype=np.float32)
    ice=np.array((.86,.95,.915),dtype=np.float32)
...
def _broad_bundles(collection,surface,inner,outer,materials):
    ...
    for k in range(primary):
        ...
        t=rng.uniform(.018,.28) if k%4 else rng.uniform(.44,.71)
        r0=inner+(outer-inner)*t
        r1=min(outer-.00040,r0+rng.uniform(.0048,.0126))
        drift=rng.uniform(-.00105,.00105)
        width=rng.uniform(.00090,.00330)
        level=rng.uniform(.000035,.000073)
        idx=rng.choices((8,9,10),(8,3,3))[0]
        _append_bundle(verts,faces,ids,surface,inner,outer,r0,r1,a,drift,width,level,idx,rng)
        if rng.random()<.62:
            rb=r0+(r1-r0)*rng.uniform(.40,.67)
            end=min(outer-.00040,rb+rng.uniform(.0031,.0078))
    ...
    ob['legacy_bundle_width_m']=[.00090,.00330]
    ob['legacy_bundle_length_m']=[.0048,.0126]
